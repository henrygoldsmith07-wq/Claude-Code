  ...
  return (
    <Card className="bg-gray-900 border-gray-800 text-white">
      ...
      <CardContent className="space-y-6">
        {currentlyPlaying && (
          <div>
            <p className="text-xs text-gray-400 mb-2 uppercase font-bold tracking-tighter">Recently Played on Spotify</p>
            <div className="flex items-center gap-3 bg-green-500/10 p-3 rounded-lg border border-green-500/20">
              <div className="w-12 h-12 bg-gray-800 rounded flex items-center justify-center text-green-500">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400">{currentlyPlaying.artist_podcast}</p>
              </div>
            </div>
          </div>
        )}
        ...
        <div>
          <p className="text-xs text-gray-400 mb-3 uppercase font-bold tracking-tighter">Recent Podcasts</p>
          <div className="space-y-2">
            {pocketCasts.length > 0 ? (
              pocketCasts.slice(0, 3).map((pod) => (
                <div key={pod.id} className="text-xs flex justify-between items-center bg-gray-800/50 p-2 rounded">
                  <div className="truncate max-w-[200px]">
                    <span className="font-medium">{pod.title}</span>
                    <span className="text-gray-500 mx-1">•</span>
                    <span className="text-gray-500">{pod.artist_podcast}</span>
                  </div>
                </div>
              ))
            ) : (
              <p className="text-xs text-gray-500 italic">No recent podcasts found</p>
            )}
          </div>
        </div>
        ...
        <div className="pt-2 border-t border-gray-800 flex justify-between text-xs text-gray-400">
          ...
          <span className="font-bold text-white">2h 15m</span>
        </div>
      </CardContent>
    </Card>
  );
