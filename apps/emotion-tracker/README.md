...
"angry" or "sad", you describe a situation and Gemini asks follow-up
...
2. Gemini asks one probing question at a time (at least 3, at most 5) to dig
   past the first label — anger is often hurt, fear, shame, or insecurity in
   disguise.
3. Gemini is deliberately instructed to avoid confirming your framing. It
   checks for self-serving bias, mind-reading, catastrophizing, and moral
   licensing ("they wronged me, so I'm entitled to X") — being wronged by
   someone doesn't make every reaction to it justified, and the model is
   instructed to say so directly rather than just validate you.
...
Set `GEMINI_API_KEY` in `.env.local` for a server-wide fallback key, or
