...
- **Longitudinal validation** (`src/lib/validationMetrics.ts`) — answers five questions descriptively from local data: does a pattern remain supported later; how often patterns carry contradictory evidence; how often incorrect patterns get retired (user rejection or staleness expiry); whether user corrections permanently stop rejected interpretations (`correctionPersistence`); and whether confidence corresponds to later support (calibration report).
