...
function pickParser(argv, output) {
  ...
  if (joined.includes('go test') || /\bgo\b/.test(joined) && joined.includes('test')) return PARSERS.gotest;
  if (joined.includes('gradle') || joined.includes('./gradlew')) return PARSERS.gradle;
  if (joined.includes('mvn ') || joined.includes('maven')) return PARSERS.maven;
  ...
  if (output) {
    ...
    if (/: error:/i.test(out) && /Success: no issues/i.test(out) === false && out.includes('reveal_type') === false) {
      if (/: error:/i.test(out) && /note:/i.test(out)) {
        // mypy vs ruff: ruff has rule codes like F401/E501
        if (/\b[A-Z]\d{3}\b/.test(out)) return PARSERS.ruff;
        return PARSERS.mypy;
      }
    }
...
module.exports = { PARSERS, pickParser, listParsers };
