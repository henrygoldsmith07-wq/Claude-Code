...
function finding(overrides: Partial<Finding> = {}): Finding {
  return {
    id: "finding-test",
    candidateId: "cand-study-exercise",
...
describe("InsightHistory ledger", () => {
  it("records the first scan with every insight marked appeared", () => {
    ...
    expect(entry.id).toBe("cand-study-exercise");
  ...
  it("records a disappearance without a note when the rejection log does not explain it", () => {
    ...
    history.recordScan(scan("2025-07-01T00:00:00.000Z", [finding()]));
    history.recordScan(scan("2025-08-01T00:00:00.000Z", [], [
      { candidateId: "cand-recall-exercise", outcomeMetricId: "study.recall_grade", exposureMetricId: "exercise.volume", reason: "Unrelated rejection" },
    ]));
  ...
  it("keeps distinct questions apart even when they share a relationship signature", () => {
    const history = new InsightHistory();
    const evening = (id: string, value: number, n: number): Finding =>
      finding({
        id,
        candidateId: "cand-qacc-evening",
        metricIds: ["study.question_accuracy"],
        title: "Question accuracy is higher evening sessions",
        sampleSize: n,
        effect: { kind: "hedges_g", value, magnitude: "small", label: "+0.40 SD" },
        tags: ["time-of-day", "study"],
      });
    const practice = (id: string, value: number, n: number): Finding =>
      finding({
        id,
        candidateId: "cand-qacc-practice",
        metricIds: ["study.question_accuracy"],
        title: "Question accuracy is higher method = practice",
        sampleSize: n,
        effect: { kind: "hedges_g", value, magnitude: "small", label: "+0.40 SD" },
        tags: ["attribute-split", "study"],
      });

    history.recordScan(scan("2025-07-01T00:00:00.000Z", [evening("e1", 0.4, 200)]));
    history.recordScan(scan("2025-08-01T00:00:00.000Z", [practice("p1", 0.3, 300)]));
    // Both questions present in one scan: the time-of-day finding must not be
    // misread as a re-appearance of the attribute-split one.
    history.recordScan(scan("2025-09-01T00:00:00.000Z", [evening("e2", 0.41, 250), practice("p2", 0.31, 350)]));

    const entries = history.history();
    expect(entries).toHaveLength(2);
    const byId = new Map(entries.map((entry) => [entry.id, entry]));
    expect(byId.get("cand-qacc-evening")!.episodes.map((episode) => episode.change)).toEqual([
      "appeared",
      "disappeared",
      "appeared",
    ]);
    expect(byId.get("cand-qacc-practice")!.episodes.map((episode) => episode.change)).toEqual([
      "appeared",
      "unchanged",
    ]);
  });
