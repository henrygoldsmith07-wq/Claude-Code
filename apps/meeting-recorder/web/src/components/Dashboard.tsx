  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        {meetings && meetings.length > 0 && (
          <p className="text-xs text-white/40 tabular">
            {filtered?.length ?? 0} of {meetings.length}
          </p>
        )}
      </div>
      ...
      {meetings && meetings.length > 0 && (
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
          <input
            ...
            className="w-full rounded-lg border border-edge bg-panel px-3 py-2 text-sm outline-none focus:border-white/30 sm:max-w-xs"
            aria-label="Search meetings"
          />
          <select
            value={statusFilter}
            onChange={(e) => setStatusFilter(e.target.value as typeof statusFilter)}
            className="rounded-lg border border-edge bg-panel px-3 py-2 text-sm outline-none focus:border-white/30"
            aria-label="Filter by status"
          >
            <option value="all">All statuses</option>
            <option value="ready">Ready</option>
            <option value="processing">Transcribing</option>
      ...
      {filtered && filtered.length === 0 && meetings && meetings.length > 0 && (
        <div className="rounded-xl border border-dashed border-edge p-8 text-center text-sm text-white/40">
