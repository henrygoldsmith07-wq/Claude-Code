...
function SafetyView() {
  ...
  return (
    <>
      <Card>
        <p className="text-[12px] font-bold uppercase tracking-wide" style={{ color: 'var(--faint)' }}>What this leaves you</p>
        <p className="mt-0.5 text-[22px] font-extrabold">
          {left.toLocaleString()} <span className="text-[13px] font-semibold" style={{ color: 'var(--muted)' }}>of {total.toLocaleString()} dishes</span>
        </p>
      ...
      <Card className="space-y-2.5">
        <div className="flex items-center justify-between">
          <p className="font-bold text-[14px]">Allergies</p>
        ...
        {app.allergies.length > 0 && (
          <div className="rounded-2xl p-3" style={{ background: 'var(--card-2)' }}>
            <p className="text-[12px] font-bold uppercase tracking-wide mb-1" style={{ color: 'var(--faint)' }}>Where these hide</p>
            {app.allergies.map((id) => {
              const found = ALLERGENS.find((a) => a.id === id);
              if (!found?.also?.length) return null;
              return (
                <p key={id} className="text-[12px] font-semibold" style={{ color: 'var(--muted)' }}>
      ...
      <Card className="space-y-2.5">
        <p className="font-bold text-[14px]">Religious & cultural</p>
        <p className="text-[12.5px] font-semibold" style={{ color: 'var(--muted)' }}>
      ...
      <Card className="space-y-2.5">
        <p className="font-bold text-[14px]">Intolerances</p>
        <p className="text-[12.5px] font-semibold" style={{ color: 'var(--muted)' }}>
...
function TasteView() {
  ...
  return (
    <>
      <Card className="space-y-2.5">
        <p className="font-bold text-[14px]">How you eat</p>
        <div className="flex flex-wrap gap-2">
          {DIET_PATTERNS.map((d) => (
      ...
      <Card className="space-y-2.5">
        <p className="font-bold text-[14px]">Cuisines you like</p>
        <div className="flex flex-wrap gap-2">
          {CUISINES.map((c) => (
      ...
      <Card className="space-y-2.5">
        <p className="font-bold text-[14px]">How much cooking do you fancy?</p>
        ...
        <p className="text-[12px] font-bold uppercase tracking-wide pt-1" style={{ color: 'var(--faint)' }}>Time you usually have</p>
        <div className="flex gap-2 overflow-x-auto no-scrollbar">
          {TIME_BUDGETS.map((t) => (
      ...
      {UNIT_CHOICES.map((choice) => (
        <Card key={choice.key} className="space-y-2">
          <div className="flex items-center justify-between gap-3">
            <p className="font-bold text-[14px]">{choice.label}</p>
            <p className="text-[13px] font-extrabold">{shown[choice.key]}</p>
          </div>
          <div className="flex gap-2 overflow-x-auto no-scrollbar">
