    ...
    <div className="pb-6 space-y-6">
      ...
      <div className="hero-gradient px-5 pt-14 pb-2">
        ...
        <div className="mt-4 flex gap-2 rise rise-1">
          <Pill tone="accent">🔥 {app.streak}-day cooking streak</Pill>
          <Pill tone="muted">⭐ Level {app.level} · {app.xp.toLocaleString()} XP</Pill>
        </div>
      </div>
          ...
          {MEAL_SLOTS.map(({ key, label, time }) => {
            ...
            return r ? (
              ...
              <Card key={key} className="flex items-center gap-3 !p-3" onClick={() => goTab('plan')}>
                <div className="h-14 w-14 rounded-xl flex items-center justify-center text-2xl shrink-0" style={{ background: 'var(--card-2)' }}>➕</div>
                <div>
      ...
      <div className="px-5 grid grid-cols-2 gap-3 rise rise-2">
        <Card>
          ...
          <div className="mt-2 flex flex-wrap gap-1.5" role="group" aria-label="Water glasses">
            {Array.from({ length: 8 }, (_, i) => (
              <button
                key={i}
                onClick={() => app.set({ water: i + 1 === app.water ? i : i + 1 })}
                className="press text-[17px]"
                style={{ opacity: i < app.water ? 1 : 0.25 }}
        ...
        <Card>
          ...
          <p className="mt-1.5 font-bold text-[14px]">{WEEKLY_CHALLENGE.emoji} {WEEKLY_CHALLENGE.name}</p>
        ...
        <div className="flex gap-3 overflow-x-auto no-scrollbar -mx-5 px-5 snap-x">
          {AI_SUGGESTIONS.map((s) => (
            ...
            >
              ...
              <p className="mt-1.5 font-bold text-[14px] leading-snug">{s.title}</p>
        ...
        <Card>
          ...
          {expiring.length > 0 && (
            <div className="mt-3 pt-3 border-t" style={{ borderColor: 'var(--line)' }}>
              <p className="text-[12px] font-bold mb-2" style={{ color: 'var(--danger)' }}>⏰ Use soon</p>
              <div className="flex gap-2 flex-wrap">
                {expiring.slice(0, 4).map((p) => (
                  <Pill key={p.id} tone={p.expiryDays <= 1 ? 'danger' : 'warn'}>
                    {p.emoji} {p.name} · {p.expiryDays}d
                  </Pill>
                ))}
              </div>
            </div>
          ...
          <div className="mt-3 pt-3 border-t" style={{ borderColor: 'var(--line)' }}>
            ...
            <div className="flex gap-2 flex-wrap">
              {RUNNING_LOW.map((p) => (
                <Pill key={p.name} tone="muted">{p.emoji} {p.name}</Pill>
              ))}
            </div>
          </div>
        </Card>
        ...
        <div className="grid grid-cols-2 gap-3">
          {LEFTOVERS.map((l) => (
            <Card key={l.name} className="!p-3">
              <p className="font-bold text-[14px]">{l.emoji} {l.name}</p>
              <p className="text-[12px] font-semibold" style={{ color: 'var(--muted)' }}>
      ...
      <Section title="Recipe of the day" className="rise rise-4">
        <Card onClick={() => openRecipe(recipeOfDay)} className="!p-0 overflow-hidden">
          <FoodArt recipe={recipeOfDay} className="h-36 w-full" size="text-7xl" />
          <div className="p-4">
            <div className="flex items-center justify-between">
              <p className="font-extrabold text-[16px]">{recipeOfDay.name}</p>
              <Pill tone="accent">★ {recipeOfDay.rating}</Pill>
            </div>
