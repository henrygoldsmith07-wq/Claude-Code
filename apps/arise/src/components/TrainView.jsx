...
import { PROGRAMS, PROGRAM_BY_ID, availablePrograms, EXERCISE_BY_ID } from '../lib/data.js';
  ...
  return (
    <div className="px-4 py-5 space-y-4">
      <div>
        ...
        <p className="text-xs text-ink3">Programs are scheduled training — picking one creates dated sessions you can run from Today.</p>
      </div>
        ...
        {PROGRAMS.map(p=>{
          ...
          return (
            <button key={p.id} onClick={()=> setProgramId(p.id)}
              ...
              <span className={`block mt-2 font-bold ${programId===p.id?'text-bg':'text-ink'}`}>{p.name}</span>
      ...
      {program && (
        <div className="space-y-3">
          <h3 className="text-sm font-bold">Preview — {program.name}</h3>
      ...
      {active && (
        <div className="rounded-2xl border border-line bg-surface p-4 space-y-2">
          ...
          <p className="text-xs text-ink3">Start: {active.startDateISO} • {active.sessions.length} sessions</p>
          <ul className="space-y-1.5 max-h-72 overflow-auto pr-1">
            {active.sessions.map(s=> (
              <li key={s.id} className="flex items-center gap-2 text-sm border border-line rounded-xl px-3 py-2 bg-surface2">
                <span className="text-xs font-mono tabular-nums text-ink3 w-24 shrink-0">{s.dateISO}</span>
                <span className="font-medium truncate">{s.title}</span>
                <span className={`ml-auto text-[11px] font-bold px-2 py-1 rounded-full border ${s.status==='done' ? 'bg-success text-white border-success' : 'bg-surface border-line text-ink3'}`}>{s.status}</span>
                {s.status!=='done' && <button onClick={()=> onStartSession(s)} className="text-xs font-bold text-ink underline">Start</button>}
              </li>
            ))}
          </ul>
        </div>
