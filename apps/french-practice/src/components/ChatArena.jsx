      ...
      {(hint || hintLoading) && (
        <div className="mx-4 sm:max-w-2xl sm:mx-auto sm:w-full mb-2 fade-in rounded-xl bg-surface2 border border-line px-3 py-2">
          {hintLoading
            ...
            : <p className="text-xs text-ink2"><span className="font-bold">💡 Indice {hintLevel}/3 :</span> {hint}</p>}
        </div>
      )}
        ...
        {recorder.recording ? (
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-center gap-4">
              ...
              <span className="text-[11px] text-ink3 w-20">3,5 s de silence = envoi auto</span>
            </div>
          </div>
        ) : phase === 'editing' ? (
          <div className="space-y-2 fade-in">
            <p className="text-[11px] text-ink2 font-medium">✏️ Vérifiez votre transcription avant l'envoi :</p>
            <textarea
              value={draft}
          ...
          <div className="flex items-end gap-2">
            ...
            <div className="flex-1 flex items-center gap-2 bg-surface2 rounded-xl border border-line focus-within:border-ink px-3">
              ...
              />
              {draft.trim() && (
                <button onClick={() => send(draft)} disabled={busy} aria-label="Envoyer" className="text-ink font-bold px-1 min-h-11">→</button>
              )}
            </div>
            <button
  ...
  return (
    <div className="flex items-end gap-2 max-w-[88%] sm:max-w-[75%] bubble-in">
      ...
      <div className="bg-surface2 rounded-2xl rounded-bl-md px-4 py-3 space-y-2">
        <p className="text-[15px] text-ink leading-relaxed">{text}</p>
    ...
    <div className="flex flex-col items-end gap-1.5 bubble-in">
      <div className="flex items-end gap-2 max-w-[88%] sm:max-w-[75%]">
        <div className="bg-accent rounded-2xl rounded-br-md px-4 py-3 shadow-md shadow-black/15">
          <p className="text-[15px] text-onaccent leading-relaxed">{turn.userText}</p>
        </div>
      ...
      {expanded && (
        <div className="w-full sm:max-w-[85%] fade-in bg-surface2 border border-line rounded-2xl p-4 space-y-3 text-left">
          ...
          <div>
            <h4 className="text-[11px] font-bold uppercase tracking-wider text-ink mb-1">Comme un natif 🇫🇷</h4>
            <p className="text-[13px] text-ink italic">{evaluation.native_alternative}</p>
