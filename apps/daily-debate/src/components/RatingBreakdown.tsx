  ...
  return (
    <div className="surface-card flex flex-col gap-4 p-5">
      <div className="flex items-baseline justify-between">
        ...
        <p className="tabular text-xs text-zinc-500">{ratings.roundsScored} rounds scored</p>
      </div>
        ...
        {FACTORS.map((factor) => {
          ...
          return (
            <div key={factor} className="flex items-center gap-3">
              <span className="w-20 shrink-0 text-xs text-zinc-400">{LABELS[factor]}</span>
              <div className="h-2 flex-1 overflow-hidden rounded-full bg-black/30 shadow-[inset_0_1px_2px_rgba(0,0,0,0.4)]">
                <div
                  className="h-full rounded-full transition-[width]"
                  style={{ width: `${(value / 10) * 100}%`, background: barColor(value) }}
                />
              </div>
              <span className="tabular w-10 shrink-0 text-right text-xs text-zinc-400">{value.toFixed(1)}</span>
            </div>
          );
        })}
      </div>
